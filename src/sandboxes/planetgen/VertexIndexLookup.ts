...
export function vertexIndexLookup(resolution: number): VertexIndexLookup {
  ...
  function leftFaceVertexIndex(row: number, col: number) {
    if (row === 0)
      return mainFaceVertexIndex(0, col, 0);
    if (row === faceSize)
      return mainFaceVertexIndex(2, faceSize - col, 0);
    if (col === 0)
      return mainFaceVertexIndex(3, faceSize - row, 0);
    if (col === faceSize)
      return mainFaceVertexIndex(1, row, 0);
  ...
  function rightFaceVertexIndex(row: number, col: number) {
    if (row === 0)
      return mainFaceVertexIndex(0, faceSize - col, faceSize);
    if (row === faceSize)
      return mainFaceVertexIndex(2, col, faceSize);
    if (col === 0)
      return mainFaceVertexIndex(1, row, faceSize);
    if (col === faceSize)
      return mainFaceVertexIndex(3, faceSize - row, faceSize);
