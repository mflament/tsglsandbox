...
export abstract class PlanetIndexBuffer {
  ...
  static create(maxResolution: number, mode: DrawMode): PlanetIndexBuffer {
    if (mode === DrawMode.TRIANGLES)
      return new TrianglesPlanetIndexBuffer(maxResolution);
    else if (mode == DrawMode.TRIANGLE_STRIP)
      return new TriangleStripPlanetIndexBuffer(maxResolution);
    else
      throw new Error("Unsupported draw mode " + DrawMode[mode]);
  }

  static indexCount(resolution: number, mode: DrawMode): number {
    if (mode === DrawMode.TRIANGLES)
      return TrianglesPlanetIndexBuffer.indexCount(resolution);
    if (mode === DrawMode.TRIANGLE_STRIP)
      return TriangleStripPlanetIndexBuffer.indexCount(resolution);
    else
      throw new Error("Unsupported draw mode " + DrawMode[mode]);
  }
