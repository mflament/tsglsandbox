...
export interface PlanetIndexGenerator {
  generate(settings: PlanetGeneratorSettings, indexBuffer?: PlanetIndexBuffer): PlanetIndexBuffer | CancellablePromise<PlanetIndexBuffer>;
}
...
export function newIndexGenerator(drawMode: DrawMode, indexPerChunk = INDEX_PER_CHUNK): PlanetIndexGenerator {
  if (drawMode === DrawMode.TRIANGLES)
    return new TrianglesIndexGenerator(indexPerChunk);
  else if (drawMode === DrawMode.TRIANGLE_STRIP)
    return new TriangleStripIndexGenerator(indexPerChunk);
  else
    throw new Error("Invalid draw mode " + drawMode);
}
...
abstract class AbstractIndexGenerator implements PlanetIndexGenerator {
  ...
  async generate(settings: PlanetGeneratorSettings, indexBuffer?: PlanetIndexBuffer): CancellablePromise<PlanetIndexBuffer> {
  ...
  protected abstract newGenerator(resolution: number, indexBuffer: PlanetIndexBuffer): Generator<unknown, PlanetIndexBuffer>;
}
