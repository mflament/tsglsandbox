...
export interface PlanetVertexGenerator {
  generate(
    ...
    vertexBuffer?: PlanetVertexBuffer
  ): PlanetVertexBuffer | CancellablePromise<PlanetVertexBuffer>;
}
...
export class DefaultVertexGenerator implements PlanetVertexGenerator {
  ...
  async generate(
    settings: PlanetGeneratorSettings,
    vertexBuffer?: PlanetVertexBuffer
  ): CancellablePromise<PlanetVertexBuffer> {
    ...
    if (vertexBuffer.capacity < vertexCount) vertexBuffer = PlanetVertexBuffer.create(resolution);
  ...
  private *newGenerator(
    resolution: number,
    elevator: ElevationModifier | undefined,
    vertexBuffer: PlanetVertexBuffer
  ): Generator<unknown, PlanetVertexBuffer> {
    ...
    for (let face = 0; face < 4; face++) {
      for (let y = 0; y < faceRows; y++) {
        for (let x = 0; x < resolution; x++) {
          addVertex(face, x, y);
          if (vertexBuffer.vertexCount % vertexPerChunk === 0) yield;
        }
      }
    }
